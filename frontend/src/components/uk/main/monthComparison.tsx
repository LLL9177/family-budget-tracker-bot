...
export default function MonthComparison_uk({
  data,
  caption,
  monthOneName,
  monthTwoName,
  ...
  return (
    <div className="mt-2 h-87 w-full rounded-xl bg-gradient-to-t from-primary/5 to-card">
      <Table className="h-full overflow-x-scroll bg-[rgba(0,0,0,0)]">
        ...
        <TableHeader>
          <TableHead>Властивість</TableHead>
        ...
        <TableBody>
          ...
          <TableRow>
            <TableCell>Загалом зароблено</TableCell>
          ...
          <TableRow>
            <TableCell>Найбільше витрачено на</TableCell>
            <TableCell>{data.current.mostSpentOn ?? "-"}</TableCell>
            <TableCell>{data.prev.mostSpentOn ?? "-"}</TableCell>
            <TableCell>Не можливо порівняти</TableCell>
          </TableRow>
          <TableRow>
            <TableCell>Найбільше зароблено з</TableCell>
